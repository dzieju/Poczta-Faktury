...
Original file: https://github.com/dzieju/dzieju-app2/blob/main/mail_accounts.py
"""
...
def log(message, level="INFO"):
    """Simple logging function compatible with Poczta-Faktury"""
...
class MailAccountManager:
    ...
    CONFIG_FILE = str(Path.home() / "mail_config.json")
    ...
    def load_config(self) -> bool:
        ...
        try:
            ...
            log(f"ERROR: Error loading mail configuration: {str(e)}", "ERROR")
    ...
    def save_config(self) -> bool:
        ...
        if not self.accounts:
            log(f"WARNING: Attempting to save empty account configuration", "WARNING")
        ...
        try:
            ...
            log(f"ERROR: Error saving mail configuration: {str(e)}", "ERROR")
            return False
    ...
    def edit_account(self, index: int, account_data: Dict) -> bool:
        ...
        if not self._is_valid_index(index):
            log(f"Invalid account index: {index}", "ERROR")
            return False
        ...
        if not self._validate_account(updated_account):
            log(f"ERROR: Invalid account data for account at index {index}", "ERROR")
            return False
    ...
    def remove_account(self, index: int) -> bool:
        ...
        """
        if not self._is_valid_index(index):
            log(f"ERROR: Invalid account index: {index}", "ERROR")
            return False
        
        if len(self.accounts) <= 1:
            log("ERROR: Cannot remove the last remaining account", "ERROR")
            return False
    ...
    def _migrate_from_legacy(self) -> bool:
        ...
        try:
            ...
            log(f"ERROR: Error migrating legacy configuration: {str(e)}", "ERROR")
    ...
    def _validate_account(self, account: Dict) -> bool:
        ...
        for field in required_fields:
            if field not in account or not account[field].strip():
                log(f"ERROR: Account validation failed: missing or empty field '{field}'", "ERROR")
                return False
        
        # Validate account type
        valid_types = ["exchange", "imap_smtp", "pop3_smtp"]
        if account["type"] not in valid_types:
            log(f"ERROR: Account validation failed: invalid type '{account['type']}'", "ERROR")
            return False
        
        # Validate email format (basic check)
        email = account["email"].strip()
        if "@" not in email or "." not in email:
            log(f"ERROR: Account validation failed: invalid email format '{email}'", "ERROR")
            return False
