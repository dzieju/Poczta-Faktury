...
Original file: https://github.com/dzieju/dzieju-app2/blob/main/gui/mail_config_widget.py
"""
try:
...
# Try to import optional mail libraries
try:
    ...
    def _test_exchange_connection(self, account):
        ...
        try:
            if not HAVE_EXCHANGELIB:
                self.result_queue.put({
                    'type': 'test_error',
                    'error': "Biblioteka exchangelib nie jest zainstalowana. Zainstaluj przez: pip install exchangelib"
                })
                return
    ...
    def _test_imap_smtp_connection(self, account):
        ...
        try:
            ...
            if not HAVE_IMAPCLIENT:
                self.result_queue.put({
                    'type': 'test_error',
                    'error': "Biblioteka imapclient nie jest zainstalowana. Zainstaluj przez: pip install imapclient"
                })
                return
